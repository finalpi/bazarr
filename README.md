...
- Preserve personal names, character names, nicknames, and speaker labels in their original Latin spelling when the source subtitle language is English. Repeated names and explicit speaker labels are extracted once for the full subtitle and enforced across every translation batch. Other source languages continue to use established Chinese translations or transliterations.
- Limit prompt overhead by including only names relevant to each translation batch. When a batch contains isolated name or dual-speaker formatting violations, retry only the invalid cues and retain all valid translations instead of paying to translate the full batch again.
- Exclude bracketed SDH sound and action descriptions such as `[Sighs]`, `[Laughs]`, and `[Door Closes]` from English-name preservation, allowing them to be translated into Chinese. Failed translation notifications retain the final cue-level validation reason for diagnosis.
