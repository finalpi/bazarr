...
- Chinese subtitle discovery adds SubHD and optional R3Sub providers. SubHD uses the current temporary-cookie download flow with trusted-host validation, mirror search, jittered rate limiting, and bounded retries. R3Sub uses an authenticated account for Taiwan official subtitle releases; when `R3SUB_PROXY_URL` is set, it still tries direct access first and only keeps the dedicated proxy after a Cloudflare HTTP 403 challenge.
