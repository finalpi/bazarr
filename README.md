...
- Translate with Ollama or any OpenAI-compatible API using neighboring cues for context, bilingual output, and low-priority `.llm.<language>` filenames. Manual translation offers the languages enabled in Bazarr's language settings. All targets use the same subtitle layout rules with target-language punctuation and display-width line wrapping.
...
- Keep punctuation appropriate to the target language and wrap dialogue at natural boundaries using a common 48-column display width (wide characters count as two). Two speakers in one cue keep separate ASCII `-` markers on the same line. The default 1080p bilingual style retains a larger translated line above the smaller original line.
