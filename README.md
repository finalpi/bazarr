...
- Translate with Ollama or any OpenAI-compatible API using neighboring cues for context, bilingual output, and low-priority `.llm.<language>` filenames. Manual translation offers the languages enabled in Bazarr's language settings. All targets use the same display-width line wrapping and ASS layout; Chinese targets retain the punctuation cleanup described below.
...
- Manual translation can optionally omit hearing-impaired cues. The option is off by default: obvious sound and music-playing descriptions are removed from a disposable source copy, while sung lyrics are kept for translation. The OpenAI-compatible model is instructed to omit only cues without speech or lyrics. The original source and automatic translation remain unchanged; the translated subtitle is saved without an HI tag.
...
- For Simplified and Traditional Chinese, replace commas and periods with spaces (preserving decimals and abbreviations) and omit utterance-ending question marks and exclamation marks; keep internal punctuation, quotes, and ellipses. Other target languages keep their own punctuation. All targets wrap at natural boundaries using the same 48-column display width (wide characters count as two). Two speakers in one cue keep separate ASCII `-` markers on the same line. The default 1080p bilingual style retains a larger translated line above the smaller original line.
- Prompt the model to retain recognizable Latin spelling for names and speaker labels in English source subtitles, and to use consistent established target-language forms for other sources. This is a translation preference rather than a rejecting validator, avoiding false positives from contractions and SDH descriptions.
- Series subtitle upload works without assigning a language profile first. Multiple files can be dropped anywhere on the series page using a browser-independent document drop handler, including Firefox on Windows. The form detects common filename markers such as `SC`/`CHS`, `TC`/`CHT`, and inspects UTF-8 subtitle text for simplified Chinese, traditional Chinese, Japanese, Korean, or English. It also maps unambiguous `[01]`-style episode numbers and lets every detected language or episode be corrected before upload. Without a profile, manually uploaded ASS/SRT files keep their original format while being renamed to Bazarr's standard media-language filename.
- Chinese subtitle discovery adds SubHD and optional R3Sub providers. SubHD uses the current temporary-cookie download flow with trusted-host validation, mirror search, jittered rate limiting, bounded retries, and a direct-first `SUBHD_PROXY_URL` fallback retained only after HTTP 403/429 throttling. R3Sub uses an authenticated account for Taiwan official subtitle releases; when `R3SUB_PROXY_URL` is set, it still tries direct access first and only keeps the dedicated proxy after a Cloudflare HTTP 403 challenge.
...
- Save LLM subtitles as styled ASS files with separate `Target` and `Original` styles for every language. The defaults put larger bold yellow translated text above smaller white original text; both styles and the bottom margin can be adjusted independently with a live preview under **Settings > Subtitles > LLM Subtitle Appearance**. Existing appearance settings are reused for the translated style.
